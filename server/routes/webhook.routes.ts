...
async function getActualTableNameAndFields(sql: any) {
  ...
  try {
    ...
    console.warn("[Express Webhook] Failed to dynamically inspect schema via Neon:", e.message);
  }
...
router.post("/material", async (req, res) => {
  ...
  try {
    const { materialId, status, resultUrl } = req.body;
    ...
    try {
      ...
      console.log(`[Express Webhook] Material ${materialId} successfully updated in database table "${tableName}" via Neon.`);
    } catch (neonError: any) {
      console.warn("[Express Webhook] Neon direct update failed, trying fallback Prisma raw SQL:", neonError.message);
      ...
      console.log(`[Express Webhook] Material ${materialId} successfully updated in database table "${tableName}" via Prisma fallback.`);
    }
